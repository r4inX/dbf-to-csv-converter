...
class DBFDataValidator:
    ...
    def _detect_duplicates(self):
        ...
        for idx, record in enumerate(self.records):
            ...
            record_hash = hashlib.md5(
                record_str.encode("utf-8", errors="ignore")
            ).hexdigest()
        ...
        self.validation_results["duplicates"] = {
            ...
            "duplicate_percentage": (
                (sum(dup["count"] for dup in duplicates) / self.total_records * 100)
    ...
    def _analyze_fields(self):
        ...
        for field_name, stats in field_stats.items():
            ...
            stats["fill_rate"] = (
                ((self.total_records - stats["null_count"]) / self.total_records * 100)
                if self.total_records > 0
                else 0
            )
            ...
            stats["appears_categorical"] = (
                stats["unique_count"] < self.total_records * 0.1
                and stats["unique_count"] < 100
            )

            # Pattern analysis for common formats
            stats["pattern_analysis"] = self._analyze_field_patterns(
                stats["sample_values"]
            )
    ...
    def _check_type_consistency(
        self, value, expected_type: str, record_idx: int
    ) -> Optional[Dict]:
    ...
    def _analyze_missing_data(self):
        ...
        for field in self.field_info:
            ...
            missing_percentage = (
                (total_missing / self.total_records * 100)
                if self.total_records > 0
                else 0
            )
    ...
    def _calculate_encoding_confidence(self):
        ...
        self.validation_results["encoding_confidence"] = {
            ...
            "alternative_encodings": sorted(
                confidence_scores.items(), key=lambda x: x[1], reverse=True
            ),
            "recommendation": self._get_encoding_recommendation(
                confidence_scores, self.encoding_used
            ),
        }
    ...
    def _test_encoding_quality(self, sample_text: List[str], encoding: str) -> float:
        ...
        for text in sample_text:
            try:
                ...
                quality_score += (
                    char_retention * 40
                )  # 40% weight for character retention
                if has_german_chars:
    ...
    def _get_encoding_recommendation(
        self, confidence_scores: Dict[str, float], current_encoding: str
    ) -> str:
    ...
    def _calculate_quality_score(self):
        ...
        encoding_score = self.validation_results["encoding_confidence"][
            "confidence_score"
        ]
        ...
        type_score = (
            max(0, 100 - (type_issue_count / total_fields * 100))
            if total_fields > 0
            else 100
        )
    ...
    def _generate_summary(self):
        ...
        if self.validation_results["duplicates"]["duplicate_percentage"] > 5:
            summary["recommendations"].append(
                "Consider removing duplicate records before processing"
            )
...
def validate_dbf_file(dbf_path: str, encoding: str = None) -> Dict[str, Any]:
    ...
    encodings_to_try = (
        [encoding] if encoding else ["cp1252", "iso-8859-1", "cp850", "utf-8"]
    )
